...
Secrets for `hacker-house-medellin/hhm-cli` are **committed, encrypted**, with [sops] + [age],
...
- **No multi-line values.** Mount a PEM as a secret file and point the process
  at it: `JWT_PRIVATE_KEY_FILE=/run/secrets/jwt-signing-key.pem`
...
```sh
just env-docker-run dev ghcr.io/hacker-house-medellin/hhm-cli:dev

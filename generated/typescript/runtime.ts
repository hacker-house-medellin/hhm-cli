...
export function loadFrom(lookup: (key: string) => string | undefined): CliEnvValues {
  return {
    base_url: nonEmpty(lookup("HHM_BASE_URL")) ?? "http://127.0.0.1:8080",
    output: nonEmpty(lookup("HHM_OUTPUT")) ?? "json",
    timeout_seconds: parseIntValue(lookup("HHM_TIMEOUT_SECONDS"), 20),
...
export function loadEnvMap(
  ...
  const out: Record<string, string> = {};
  const base_url = pick(["HHM_BASE_URL"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "http://127.0.0.1:8080");
  if (base_url !== undefined) out["HHM_BASE_URL"] = base_url;
  const output = pick(["HHM_OUTPUT"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "json");
  if (output !== undefined) out["HHM_OUTPUT"] = output;
  const timeout_seconds = pick(["HHM_TIMEOUT_SECONDS"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "20");
  if (timeout_seconds !== undefined) out["HHM_TIMEOUT_SECONDS"] = timeout_seconds;
  const id = pick(["HHM_ID"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, undefined);
  if (id !== undefined) out["HHM_ID"] = id;
  return out;
}

...
final class CliEnvValues {
  const CliEnvValues({required this.baseUrl, required this.output, required this.timeoutSeconds, required this.id});
...
CliEnvValues loadFrom(String? Function(String key) lookup) {
  return CliEnvValues(
    baseUrl: _nonEmpty(lookup('HHM_BASE_URL')) ?? 'http://127.0.0.1:8080',
    output: _nonEmpty(lookup('HHM_OUTPUT')) ?? 'json',
    timeoutSeconds: _parseInt(lookup('HHM_TIMEOUT_SECONDS'), 20),
    id: _nonEmpty(lookup('HHM_ID')),
  );
}
...
Map<String, String> loadEnvMap(
  ...
  final out = <String, String>{};
  final baseUrl = pick(['HHM_BASE_URL'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, 'http://127.0.0.1:8080');
  if (baseUrl != null) out['HHM_BASE_URL'] = baseUrl;
  final output = pick(['HHM_OUTPUT'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, 'json');
  if (output != null) out['HHM_OUTPUT'] = output;
  final timeoutSeconds = pick(['HHM_TIMEOUT_SECONDS'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, '20');
  if (timeoutSeconds != null) out['HHM_TIMEOUT_SECONDS'] = timeoutSeconds;
  final id = pick(['HHM_ID'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, null);
  if (id != null) out['HHM_ID'] = id;
  return out;
}

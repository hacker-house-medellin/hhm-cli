...
mod tests {
    ...
    fn cli_values_override_environment_values() {
        let initial = EnvMap::from([
            ("HHM_OUTPUT".into(), "text".into()),
            ("HHM_BASE_URL".into(), "http://127.0.0.1:8080".into()),
        ]);
        let overrides = EnvMap::from([("HHM_OUTPUT".into(), "json".into())]);
        let env = get_env_map(initial, overrides);

        assert_eq!(env.get("HHM_OUTPUT").map(String::as_str), Some("json"));
        assert_eq!(
            env.get("HHM_BASE_URL").map(String::as_str),
